...
from ._dm_meta_info import ATTRIBUTE, REFERENCE_CLASS, REFERENCE_LIST, REFERENCE_LEAFLIST 
from ._dm_meta_info import REFERENCE_LIST, REFERENCE_IDENTITY_CLASS, REFERENCE_ENUM_CLASS
...
CRUD_OPERATION_MAP = {'create': 'CREATE', 'delete': 'DELETE', 'read': 'READ'}
...
class DmTree(object):
    ...
    def _to_real_list_type(self, elems, member, entity):
        if isinstance(elems, list):
            results = []
            for elem in elems:
                result = DmTree._to_real_type_helper(elem, member, entity)
                results.append(result)
            return results
        else:
            return DmTree._to_real_type_helper(elems[0], member, entity)
    ...
    @staticmethod
    def _bind_to_object(payload, entity, pretty_p='|-'):
    ...
    @staticmethod
    def _bind_to_object_helper(root, entity, pretty_p='|-'):
        if root is None:
            return

        entity_members = entity._meta_info().meta_info_class_members
        for member in entity_members:
            ...
            if member.mtype == ATTRIBUTE:
                entity.__dict__[member.presentation_name] = DmTree._to_real_type(rt, member, entity)
            elif member.mtype == REFERENCE_LEAFLIST:
                entity.__dict__[member.presentation_name] = DmTree._to_real_list_type(rt, member, entity)
                ...
                entity.__dict__[member.presentation_name] = DmTree._to_real_union_type_helper(rt, member,entity)
...
def match_node(rt, segs, sc, rcd):
    node = rt.tag.split('[')[0]
    node = node.split(':')[-1]
    if sc == 0 and rt.tag == node:
        rcd.append(rt)
        if ((len(segs)-1) == sc):
            return rt
        sc = sc + 1
    ...
    for c in rt:
        ...
        if c.tag == node:
            ...
            if ((len(segs)-1) == sc):
                return c
            match_node(c, segs, sc+1, rcd)
    return None

def walk_path(rt, cmnpath):
    segs = cmnpath.split('/')
    if segs[0] == '':
        segs = segs[1:]
    nodes = []
    match_node(rt, segs, 0, nodes)
    lp = -1
    if len(nodes) > 1:
        lp = -2
    return nodes[lp]

def collect_elems(rt, lst):
    lsts = []
    for c in rt:
        if c.tag in [lst]:
            lsts.append(c)
...
def dm_attach_wrapper(func):
    def wrapper(self, object, *__args, **__kw):
        ref_name = type(self).__name__ + '_ref'
        if ref_name in dir(object):
            object.__dict__[ref_name] = self
            if self._base_path != '/':
                self._common_path = object._common_path + self._base_path
            else:
                self._common_path = object._common_path
        else:
            raise TypeError("No compatible type reference in host.")
        func(self, object, *__args, **__kw)
    return wrapper
